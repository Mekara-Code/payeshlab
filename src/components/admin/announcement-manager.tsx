...
import { deleteAnnouncement, saveAnnouncement, toggleAnnouncementStatus, type AnnouncementActionState } from "@/app/admin/announcements/actions";
...
function formatPersianDate(value: string) {
  return new Intl.DateTimeFormat("fa-IR-u-ca-persian", { day: "numeric", month: "short", year: "numeric" }).format(new Date(value));
}
    ...
    <div className="pb-8">
      <form action={formAction} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_14px_34px_rgba(15,23,42,0.05)] sm:p-7">
        ...
        <div className="mt-6 grid gap-5">
          <label className="grid gap-2 text-sm font-black text-slate-950">عنوان اطلاعیه<input className="min-h-12 rounded-xl border border-slate-200 px-4 text-sm font-medium text-slate-800 outline-none transition focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" maxLength={200} name="title" onChange={(event) => setTitle(event.target.value)} placeholder="عنوان کوتاه و روشن اطلاعیه" required value={title} /></label>
          <label className="grid gap-2 text-sm font-black text-slate-950">توضیحات کامل<textarea className="min-h-40 resize-y rounded-xl border border-slate-200 p-4 text-sm font-medium leading-7 text-slate-800 outline-none transition focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" maxLength={10_000} name="description" onChange={(event) => setDescription(event.target.value)} placeholder="متنی که کاربران در مودال اطلاعیه می‌خوانند…" required value={description} /></label>
        </div>
